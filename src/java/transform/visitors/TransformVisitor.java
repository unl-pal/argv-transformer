...
public class TransformVisitor extends ASTVisitorUtil {
  ...
  /**
   * On a method containing a disallowed ClassInstanceCreation (such as new
   * DisallowedType()), removing it if it's in
   * an initializer or right hand side, and replacing it with a blank object if in
   * a return statement.
   */
  @Override
  ...
  /**
   * Initializes AST and symbolTableStack and pushes root to symbolTableStack.
   * Additional changes handled in endVisit
   */
  @Override
  ...
  /**
   * Adds necessary imports for the symbolic testing tool we are using. Currently
   * supports java Random and nasa's symbc.Debug
   * Removes imports not in Java Standard Library and ensures comments are
   * preserved.
   */
  @Override
  public void endVisit(CompilationUnit node) {
    ...
    for (Object typeObj : node.types()) {
      if (typeObj instanceof TypeDeclaration) {
        ...
        int startPosition = typeDecl.getJavadoc() != null ? typeDecl.getJavadoc().getStartPosition()
  ...
  /**
   * Checks if then or else expression in a conditional is not of an allowed type,
   * replacing it with a symbolic value of
   * the type of the variable declaration. Also replaces the conditional statement
   * with a symbolic boolean if it is not boolean type code.
   */
  @Override
  ...
  public void endVisit(InfixExpression node) {
    ...
    if ((lhsType == null || !typeChecker.allowedType(lhsType))
      ...
      if (TypeResolutionUtils.isIntegerTypeCode(rhsType)) {
        ...
        TypeResolutionUtils.replaceDouble(lhs, target, ast, rewriter, randUsedInMethod); // TODO: figure out why this
                                                                                         // thinks doubles are floats
  ...
  /**
   * Removes MethodDeclarations that are mains or not part of typeChecker's
   * allowed types. See typeChecker for a longer description.
   * Configures global variable currMethod to this node's method and pushes this
   * method's scope to the top of symbolTableStack
   */
  @Override
...
//      }
  ...
  /**
   * Checks if a MethodInvocation node is part of the JDK or package environment,
   * replacing/removing it if otherwise.
...
//        }
  ...
  public void endVisit(QualifiedName node) {
    ...
    if (node.getLocationInParent() == VariableDeclarationFragment.INITIALIZER_PROPERTY) {
      ...
      TypeResolutionUtils.replaceArgumentWithinMethodInvocation(node, (MethodInvocation) node.getParent(), rewriter,
          ast, randUsedInMethod);
  ...
  /**
   * If the return type is resolveable and not of an allowed type, replaces it
   * with a new blank Object
   * If resolvable and of an allowed type but not in typeTable, replaces it with a
  ...
  public void endVisit(ReturnStatement node) {
    ...
    if (!typeChecker.allowedType(type)) {
      ...
      rewriter.replace(expr, TypeResolutionUtils.createSymbolicArgument(returnTypeBinding, ast, randUsedInMethod),
          null);
    }
  }
    ...
    if (sym != null && sym.isFieldVar() && !initializedVars.contains(sym)) {
      ...
      if (type.isPrimitiveType()) {
        if (TypeResolutionUtils.isIntegerTypeCode(type)) {
          ...
          Expression randMethodInvocation = TypeResolutionUtils.generateIntegerFromTarget(ast, randUsedInMethod,
              target);
          ...
          Expression randMethodInvocation = TypeResolutionUtils.generateBooleanFromTarget(ast, randUsedInMethod,
              target);
...
//        }
  ...
  /**
   * If a simpleName node is not of an allowed type and belongs to an if
   * statement, replaces it with a symbolic boolean.
  ...
  public void endVisit(SuperMethodInvocation node) {
    if (node.getLocationInParent() == IfStatement.EXPRESSION_PROPERTY) {
      ...
      // if the type directly above is a cast, we can ignore the need for it as it is
      // implicit in the new symbolic value
      if (TypeResolutionUtils.isIntegerTypeCode(type)) {
  ...
  /**
   * Removes Interfaces and strips and "extends" and "implements" from a class'
   * type declaration
   * Pushes a new scope to the symbol table for the class we are about to enter
  ...
  /**
   * If a VariableDeclarationFragment is missing an initializer, an initalizer is
   * added
   */
  @Override
   ...
   * If its parent is not a block, instead of directly removing the
   * VariableDeclarationStatement,
   * it is replaced with an empty block "{}". Note: This can create issues for
   * some statements
   */
  @Override
